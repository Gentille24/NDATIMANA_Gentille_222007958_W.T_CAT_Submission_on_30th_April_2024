...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO orders(Id, quantity, item) VALUES (?, ?, ?)"); 
    $stmt->bind_param("iss", $id, $quantity, $item);
    ...
    <table border="5">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $oid = $row['id']; // Fetch the menu_Id
                echo "<tr>
                    <td>" . $row['id'] . "</td>
                    <td>" . $row['quantity'] . "</td>
                    <td>" . $row['item'] . "</td>
                      <td><a style='padding:4px' href='delete_orders.php?id={$row['id']}'>Delete</a></td>
                    <td><a style='padding:4px' href='update_orders.php?id={$row['id']}'>Update</a></td>
                </tr>";
            }
        } else {
