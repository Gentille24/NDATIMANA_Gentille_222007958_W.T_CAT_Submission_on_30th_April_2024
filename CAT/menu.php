...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO menu(Id, price, availability) VALUES (?, ?, ?)"); 
        $stmt->bind_param("iss", $id, $price, $availability);
...
<head>
    ...
    <title>Detail information Of menu</title>
    <style>
        table {
            width: 100%;
            border-collapse: collapse;
...
<body>
    <center><h2>Table of menu</h2></center>
    <table border="5">
        <tr>
            <th> Id</th>
            <th>price</th>
        ...
        // Check if there are any menu
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $mid = $row['id']; // Fetch the menu_Id
                echo "<tr>
                    <td>" . $row['id'] . "</td>
                    <td>" . $row['price'] . "</td>
                    <td>" . $row['availability'] . "</td>
                 <td><a style='padding:4px' href='delete_menu.php?id={$row['id']}'>Delete</a></td>
                    <td><a style='padding:4px' href='update_menu.php?id={$row['id']}'>Update</a></td>
                </tr>";
            }
        } else {
