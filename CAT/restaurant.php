...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Prepare and bind the parameters
     $stmt = $connection->prepare("INSERT INTO restaurant (Id, name, address, number) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("isss", $id, $name, $address, $number);
...
// SQL query to fetch data from the restaurant table
...
<head>
    ...
    <title>Detail information Of restaurant</title>
    <style>
        table {
            width: 100%;
            border-collapse: collapse;
...
<body>
    <center><h2>Table of restaurant</h2></center>
    <table border="5">
        <tr>
            <th> Id</th>
            <th>name</th>
        ...
        // Check if there are any rest
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $rid = $row['id']; // Fetch the restaurant_Id
                echo "<tr>
                    <td>" . $row['id'] . "</td>
                     <td>" . $row['name'] . "</td>
                    <td>" . $row['address'] . "</td>
                   <td>" . $row['number'] . "</td>
                        <td><a style='padding:4px' href='delete_restaurant.php?id={$row['id']}'>Delete</a></td>
                    <td><a style='padding:4px' href='update_restaurant.php?id={$row['id']}'>Update</a></td>
                </tr>"; 
